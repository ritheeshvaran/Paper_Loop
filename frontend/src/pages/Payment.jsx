...
const Payment = ({ settings }) => {
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/orders/${id}/submit-payment`, { transaction_id: txn.trim() });
      ...
      <div className="pl-container max-w-3xl">
        ...
        <div className="mt-10 grid md:grid-cols-2 gap-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border border-neutral-200 p-6 text-center">
            ...
            <img data-testid="gpay-qr" src={settings?.gpay_qr_url} alt="GPay QR" className="w-full max-w-xs mx-auto aspect-square object-contain bg-white" />
            <button
          ...
          <form onSubmit={submit} className="border border-neutral-200 p-6">
            ...
            />
            <p className="text-xs text-neutral-500 mt-3 flex gap-2"><CheckCircle2 className="w-4 h-4 text-[color:var(--pl-orange)]" /> Your item stays reserved for the next 30 minutes while we verify.</p>
