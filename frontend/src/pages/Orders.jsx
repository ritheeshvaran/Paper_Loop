    ...
    <div className="pl-section-light py-16 min-h-screen">
      <div className="pl-container">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {["all", "placed", "payment_under_validation", "approved", "delivered", "cancelled"].map((s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-neutral-500">No orders here yet. <Link to="/collections" className="underline">Start shopping</Link>.</div>
        ) : (
                ...
                <Link to={`/account/orders/${o.id}`} data-testid={`order-row-${o.order_number}`} className="block border border-neutral-200 p-6 hover:border-black transition-colors">
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="md:text-right">
                      ...
                      <span className={`inline-block mt-1 text-[10px] uppercase tracking-widest px-2 py-0.5 ${statusColor(o.status)}`}>{statusLabel(o.status)}</span>
                    </div>
                  </div>
                </Link>
              </li>
